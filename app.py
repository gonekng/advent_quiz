...
def login():
    ...
    if st.button('입력'):
        if df['name'].isin([name]).any():
            ...
            if st.button('퀴즈 풀러가기'):
                st.rerun()
    st.info('📢 현재 매일 아침마다 저장된 데이터가 초기화되는 이슈가 반복해서 발생하고 있습니다. 서비스 이용에 불편을 끼쳐드려 대단히 죄송합니다. 어떻게든 빠른 시일 내에 복구하도록 하겠습니다. 그 전까지는 복습한다는 마음으로 처음부터 다시 한번 쭉 풀어주시기 바랍니다. 크리스마스가 다가올수록 설렘이 짙어진다고 좋게좋게 생각해주세요. 제가 말이 좀 많죠? 양해 부탁드립니다. 오늘 하루도 힘냅시다!')
