...
def show_home():
    ...
    for i in range(7):
        ...
        cols[i].markdown(f'<div style="font-size: 17px; font-weight: bold; background-color: rgba(46, 139, 87, 0.9); text-align: center; padding: 5px; color: {color};">{weekday}</div>', unsafe_allow_html=True)   
    st.write('')
    for week in cal:
        cols = st.columns(7)
